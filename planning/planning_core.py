    ...
    def execution_prompt(base_system: str, math_verification: bool = False) -> str:
        ...
        if math_verification:
            prompt += (
                ...
                "call it before writing your final answer: check your derived "
                "formula/claim against a prior numeric result, check a logical "
                "claim's satisfiability/entailment directly, or - for a puzzle whose "
                "answer is a strategy over several cases (branches, role "
                "assignments, coin flips) - use `z3_run_script` to encode ALL cases "
                "as constraints and let Z3 check the property holds for every one of "
                "them at once. Do not verify by re-deriving the same reasoning by "
                "hand again, and do not check only one or two example cases when the "
                "claim is about every case - call the tool with the full case space. "
                "If the tool's verdict disagrees with your derivation, trust the tool "
                "and redo the derivation instead of arguing with it.\n"
            )
        return prompt
