...
`--gguf model.gguf` serves an untrained GGUF base model through llama.cpp instead (kev.gguf), and `--ollama <name>` a
model a running Ollama already holds (kev.ollama) — the same endpoints, no checkpoint, no torch device, and in the
Ollama case no second copy of the weights beside the chat UI's.
...
class Server:
    ...
    def count_output_tokens(self, answers):
        """The usage figure for a serialised answer set; every backend answers this its own way."""
        return output_tokens(self.tok, answers)

    def card(self):
        """The Kev serving details /v1/models reports next to the TypeSafe model card fields."""
        ck, meta = self.checkpoint, self.checkpoint.meta
        return {"description": f"Kev pointer head on {meta.base}, serving {ck.requested} at temperature {self.model.head.temperature:.2f}",
                "release_date": self.release_date,
                "run": ck.requested, "base": meta.base, "lora": meta.lora, "device": self.device, "backend": self.model.backend,
                "dtype": self.model.dtype, "temperature": self.model.head.temperature,
                "prefix_cache": {"size": PREFIX_CACHE_SIZE, "min_state_tokens": self.prefix_min_tokens, "hits": self.prefix_hits,
                                 "misses": self.prefix_misses, "cached_states": len(self.prefix_cache)}}
...
def server():
    """The loaded backend: a Server over a checkpoint, or a gguf.GGUFServer over a GGUF. Both answer() and card()."""
...
def systemone_separate(req: SystemOneRequest):
    ...
    return {"model": req.model, "answers": answers,
            "usage": {"input_tokens": sum(p["usage"]["input_tokens"] for p in parts), "output_tokens": server().count_output_tokens(answers)},
            "latency_ms": round(sum(p["latency_ms"] for p in parts), 1)}


@app.get("/v1/models")
def models():
    """One TypeSafe model card (name, description, release_date) per accepted model name, plus the serving details a
    client may ignore: the run or GGUF, the base, the device, the backend and precision, the temperature, cache stats."""
    card = server().card()
...
def main():
    ...
    ap.add_argument("--gguf", help="serve this GGUF base model through llama.cpp (kev.gguf) instead of a checkpoint")
    ap.add_argument("--ollama", help="serve a model a running Ollama holds (kev.ollama) instead of a checkpoint")
    ap.add_argument("--run", default="runs/kev")
    ap.add_argument("--fallback", default="runs/smoke")
    ap.add_argument("--port", type=int, default=8008)
    ap.add_argument("--host", default="10.0.0.10", help="interface to bind (0.0.0.0 for all)")
    a = ap.parse_args()
    if a.gguf or a.ollama:
        if a.gguf:
            from .gguf import GGUFServer
            app.state.server = GGUFServer.load(a.gguf)
            card = app.state.server.card()
            where = f"llama.cpp {card['llama_cpp_version']} from {card['runtime']}"
        else:
            from .ollama import OllamaServer
            app.state.server = OllamaServer.load(a.ollama)
            card = app.state.server.card()
            where = f"Ollama {card['ollama_version']} at {card['host']} ({card['scoring']} scoring)"
        print(f"serving {card['base']} via {where} on {a.host}:{a.port}")
        import uvicorn
        return uvicorn.run(app, host=a.host, port=a.port)
    ...
    print(f"serving {ck.requested} ({ck.path}) on {dev} via {model.backend} ({model.dtype}) on {a.host}:{a.port}")   # /v1/models reports the run as given, not the resolved cache path
    import uvicorn
